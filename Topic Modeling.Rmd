...
#ONLY USE THIS TO CLASSIFY JUST THOSE WE ALREADY KNOW
topicsDF = topicsDF %>% filter(primary_genre != "Unknown") 
#END ONLY USE THIS
...
top_genres = topicsDF %>% group_by(primary_genre) %>% summarize(cluster=n()) %>% mutate(rank=rank(-cluster)) %>% arrange(rank) %>% slice(2:15) %>% select(primary_genre) %>% unlist
...
#ONLY USE THIS WITH THE GENRES WE ALREADY KNOW
tidiedCheck = tidied %>% filter(primary_genre == classified_genre)
#END ONLY USE THIS

#tidied where the classified_genre is not Unknown and probability is high
tidiedLessUnknown = tidied %>% filter(primary_genre=="Unknown") %>% filter(classified_genre!="Unknown") %>% filter(probability > .15) 
...
ggplot(confusion) + geom_histogram(stat="identity") + aes(x=probability,y=`cluster`,fill=classified_genre) + coord_flip()
