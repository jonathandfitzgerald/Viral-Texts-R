...
out_of_domain_predictions =lapply(models,predict,newdata = data.frame(unclassified_data),type="response") %>% do.call(cbind,.) %>% as.data.frame
names(out_of_domain_predictions) = top_genres

out_of_domain_predictions = cbind(topicsDF %>% select(cluster,primary_genre) %>% filter(!should_be_training),predictions_frame)

tidied_ood = out_of_domain_predictions %>% gather("classified_genre","probability",-primary_genre,-cluster)

tidied_ood_unknown = tidied_ood %>% filter(primary_genre == "Unknown")
...
tidied %>% filter(primary_genre=="political") %>% ggplot() + geom_histogram(aes(x=probability)) + facet_wrap(~classified_genre,scales="free_y")
