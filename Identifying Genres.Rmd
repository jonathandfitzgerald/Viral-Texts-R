...
poems = df[grep("^poe", df$V9),] %>% as.data.frame()
...
poemswithBY = halfCluster[grep("<br/>BY", halfCluster$V9),] %>% as.data.frame()
write.csv(poemswithBY, file = paste('output/poems-with-by.csv',sep=""))
...
nameEnd = halfCluster[grep("<br/>[A-Z][-'a-zA-Z]+,?\\s[A-Z][-'a-zA-Z]{0,19}$", halfCluster$V9),] %>% as.data.frame()
